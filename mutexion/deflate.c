...
#ifdef MUTEXION
int ZEXPORT mutexionInit2_(strm, level, method, windowBits, memLevel, strategy,
                           version, stream_size, next_secret, next_userinput,
                           buf_len) z_streamp strm;
int         level;
int         method;
int         windowBits;
int         memLevel;
...
unsigned int    buf_len;
{
    ...
    // added for Mutexion
    s->next_secret = next_secret;
    s->cur_window_next_secret = s->next_secret;
    s->next_userinput = next_userinput;
    s->cur_window_next_userinput = s->next_userinput;
    s->buf_len = buf_len;
...
#ifdef MUTEXION
    if (s->cur_window_next_userinput[s->strstart] == 0) {
        userinput_flag = 1;
    }
#endif
...
#ifdef MUTEXION
        if (userinput_flag == 0) {
            max_match_len = (s->cur_window_next_userinput[s->strstart] <
                             s->cur_window_next_userinput[cur_match])
                                ? s->cur_window_next_userinput[s->strstart]
                                : s->cur_window_next_userinput[cur_match];
        } else {
            max_match_len = (s->cur_window_next_secret[s->strstart] <
                             s->cur_window_next_secret[cur_match])
                                ? s->cur_window_next_secret[s->strstart]
                                : s->cur_window_next_secret[cur_match];
        }
#if (MUTEXION_DEBUG_LEVEL >= 3)
        if (max_match_len < MAX_MATCH) {
            printf("max_match_len=%d\n", max_match_len);
        }
#endif
        strend = s->window + s->strstart + max_match_len;
#endif
...
#ifdef MUTEXION
        len = (scan > strend) ? max_match_len
                              : max_match_len - (int)(strend - scan);
...
#ifdef MUTEXION
            s->cur_window_next_secret += wsize;
            s->cur_window_next_userinput += wsize;
            // Assert((s->cur_window_next_secret - s->next_secret + 1) <=
            // s->buf_len, "next_secret out of bound");
            // Assert((s->cur_window_next_userinput - s->next_userinput + 1) <=
            // s->buf_len, "next_userinput out of bound");
#endif
            slide_hash(s);
            more += wsize;
        }
